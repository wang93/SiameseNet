...
class DenseBraidMGN(BraidMGN):
    ...
    def __init__(self, feats=256, fc=(1,)):
        ...
        self.part_braids = nn.ModuleList()
        for channel_in in channel_ins:
            self.part_braids.append(DenseLinearBraidBlock(channel_in, channel_in))

        self.braids2braid = CatBraids()
        channel_in = sum(channel_ins) * 2

        self.final_braid = DenseLinearBraidBlock(channel_in, channel_in)
        channel_in *= 2
